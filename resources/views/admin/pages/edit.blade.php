            ...
            <form action="{{ route('admin.pages.update', $page) }}" method="post">
                ...
                <div class="form-group row{{ $errors->has('uri') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-6">
                        <input type="text" name="uri"
                               ...
                               value="{{ old('uri', $page->uri) }}" required>
                    ...
                    <div class="col-md-6 offset-md-4">
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" name="hidden" class="custom-control-input"
                                   id="hidden" value="1" {{ old('hidden', $page->hidden) ? 'checked' : '' }}>
                        ...
                        <select name="template" id="template"
                            ...
                            @foreach($templates as $template)
                                <option value="{{ $template }}"
                                        {{ old('template', $page->template) == $template ? 'selected' : '' }}>
                                    {{ $template }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        <select name="layout" id="layout"
                            ...
                            @foreach($layouts as $layout)
                                <option value="{{ $layout }}"
                                        {{ old('layout', $page->layout) == $layout ? 'selected' : '' }}>
                                    {{ $layout }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div class="col-md-2">
                            <div class="form-group{{ $errors->has('order') ? ' has-error' : '' }}">
                                ...
                                <select name="order" id="order"
                                    ...
                                    <option value="child" {{ old('order') == 'child' ? 'selected' : '' }}>
                                        Child Of
                                    </option>
                                </select>
                ...
                <div class="form-group{{ $errors->has('body') ? ' has-error' : '' }}">
                    ...
                    <textarea name="body" id="body" rows="5"
                              class="form-control{{ $errors->has('body') ? ' is-invalid' : '' }}">{{ old('body', $page->body) }}</textarea>
                ...
                <div class="form-group row{{ $errors->has('usable') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-6">
                        <select name="usable" id="usable"
                            ...
                            <option value="0"{{ old('usable', $page->usable) == 0 ? ' selected' : '' }}>Disable
                            </option>
                            <option value="1"{{ old('usable', $page->usable) == 1 ? ' selected' : '' }}>Active
                            </option>
                        </select>
